...
class AppComponent extends Component {
  constructor(props) {
    ...
    this.echartsNames = ['mapOfChina'];
    this.echartsParamInit(this.echartsNames);
  }

  componentDidMount() {
    this.echartsInit(this.echartsNames);
    this.loadMap('qg', 'china');
  }

  /* eslint-disable */
  loadMap = (mapCode, mapName) => {
    jquery.getJSON('/sources/china-main-city/' + mapCode + '.json', function (data) {
	     if (data) {
       ...
       this.echartInstance.mapOfChina.setOption(option);	
       console.log('mapOfChina', option);
		 } else {
       alert('无法加载该地图');
		 } 
	});
  ...
  echartsParamInit = (names) => {
    names.forEach((name) => {
      this.echartDom[name] = null;
      this.echartDom[`${name}Style`] = {
        width: '800px',
        height: '600px'
      };
      this.echartInstance[name] = null;
    });
  }
  // echarts实例初始化函数
  echartsInit = (names) => {
    names.forEach((name) => {
      this.echartInstance[name] = echarts.init(this.echartDom[name]);
      this.echartInstance[name].setOption(this.echartOption[name]);
    });
    /* eslint-disable */
    this.echartInstance.mapOfChina.on('mouseover', function(params) {
    ...
    this.echartInstance.mapOfChina.on('mouseout', function(params) {
        var city = params.name;
        if (city == '福建' || city == '广东' || city == '广西' || city == '海南') {
            myChart.dispatchAction({
                type: 'downplay',
  ...
  render() {
    return (
      <div className="index">
        ...
        <div>{moment('2018-09').format('YYYY')}</div>
        <div
          style={this.echartDom.mapOfChinaStyle}
          ref={(child) => { this.echartDom.mapOfChina = child; }}
        />
      </div>
    );
  }
